...
- [x] Resolve pre-existing yamllint warning on `ansible/roles/f5_backup/tasks/main.yml:10` (line >140 chars; non-fatal, predates Phase 4 PR 1) — done in `f3a5f11` (also wrapped two `f5_boot_switch` lines surfaced once the ansible-lint hook was un-broken)
- [ ] Local-vs-CI test parity — `terraform apply` against `terraform/environments/lab/` with the corrected direct `class: "Device"` DO shape, then re-run `make test` to confirm the f5_postcheck drift gate stays green on a fresh state. Pre-existing drift artifact from Phase 1-era mock state (state was applied with the old `class: "DO"` wrapper shape); orthogonal to PR-A. CI is unaffected because integration tests skip without the Docker stack.
