...
def test_performance_all_stats_shape(client: TestClient) -> None:
    r = client.get("/bigip-lab-01/mgmt/tm/sys/performance/all-stats")
...
def test_ucs_save_echoes_name(client: TestClient) -> None:
    r = client.post("/bigip-lab-01/mgmt/tm/sys/ucs", json={"command": "save", "name": "backup.ucs"})
    assert r.status_code == 200
    assert r.json()["name"] == "backup.ucs"


def test_ucs_save_defaults_name_when_absent(client: TestClient) -> None:
    r = client.post("/bigip-lab-01/mgmt/tm/sys/ucs", json={"command": "save"})
    assert r.status_code == 200
...
def test_failover_toggles_state(client: TestClient) -> None:
    before = (
        client.get("/bigip-lab-01/mgmt/tm/cm/failover-status")
        .json()["entries"]
        .popitem()[1]["nestedStats"]["entries"]["status"]["description"]
    )
    r = client.post("/bigip-lab-01/mgmt/tm/sys/failover", json={"command": "run"})
    assert r.status_code == 200
    after = (
        client.get("/bigip-lab-01/mgmt/tm/cm/failover-status")
        .json()["entries"]
