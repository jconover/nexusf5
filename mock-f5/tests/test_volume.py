...
def test_volume_list_returns_both_volumes(client: TestClient) -> None:
    r = client.get("/bigip-lab-01/mgmt/tm/sys/software/volume")
...
def test_volume_get_shows_active_flag(client: TestClient) -> None:
    r = client.get("/bigip-lab-01/mgmt/tm/sys/software/volume/HD1.1")
    assert r.status_code == 200
...
def test_volume_get_unknown_404(client: TestClient) -> None:
    r = client.get("/bigip-lab-01/mgmt/tm/sys/software/volume/HD9.9")
    assert r.status_code == 404


def test_patch_active_flips_the_other_volume_to_inactive(client: TestClient) -> None:
    r = client.patch("/bigip-lab-01/mgmt/tm/sys/software/volume/HD1.2", json={"active": True})
    assert r.status_code == 200
    assert r.json()["active"] is True

    # The other volume must have flipped to inactive — exactly one boots next.
    assert client.get("/bigip-lab-01/mgmt/tm/sys/software/volume/HD1.1").json()["active"] is False
    assert client.get("/bigip-lab-01/mgmt/tm/sys/software/volume/HD1.2").json()["active"] is True
...
def test_patch_active_is_idempotent(client: TestClient) -> None:
    ...
    # to re-run safely.
    r1 = client.patch("/bigip-lab-01/mgmt/tm/sys/software/volume/HD1.1", json={"active": True})
    r2 = client.patch("/bigip-lab-01/mgmt/tm/sys/software/volume/HD1.1", json={"active": True})
    assert r1.status_code == 200
    assert r2.status_code == 200
    assert client.get("/bigip-lab-01/mgmt/tm/sys/software/volume/HD1.1").json()["active"] is True
    assert client.get("/bigip-lab-01/mgmt/tm/sys/software/volume/HD1.2").json()["active"] is False
...
def test_patch_deactivate_is_rejected(client: TestClient) -> None:
    ...
    r = client.patch("/bigip-lab-01/mgmt/tm/sys/software/volume/HD1.1", json={"active": False})
    assert r.status_code == 400


def test_patch_unknown_volume_404(client: TestClient) -> None:
    r = client.patch("/bigip-lab-01/mgmt/tm/sys/software/volume/HD9.9", json={"active": True})
    assert r.status_code == 404
